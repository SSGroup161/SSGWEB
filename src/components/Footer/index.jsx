import React from "react";

const Footer = () => {
    return (
        <>
            <div className="w-full bg-[#D2AC47] h-auto px-10 md:px-20 lg:px-36 gap-8 py-12 flex flex-wrap justify-between mt-48">
                <div className="flex flex-col justify-between h-full">
                    <div className="flex flex-col gap-2">
                        <div>
                            <svg
                                width="25"
                                height="80"
                                viewBox="0 0 314 653"
                                fill="none"
                                xmlns="http://www.w3.org/2000/svg"
                            >
                                <path
                                    d="M35.4601 427.49C53.9671 427.49 68.9701 412.487 68.9701 393.98C68.9701 375.473 53.9671 360.47 35.4601 360.47C16.953 360.47 1.95007 375.473 1.95007 393.98C1.95007 412.487 16.953 427.49 35.4601 427.49Z"
                                    fill="white"
                                />
                                <path
                                    d="M130.82 233.21C108.34 219.45 91.16 209.65 78.96 199.95C10.32 145.31 37.05 78.08 37.05 78.08C1.95003 104.38 -7.97995 156.61 8.76005 195.48C25.7401 234.92 69.3 258.86 103.68 281.11C123.21 293.75 149.57 306.44 165.64 323.85C188.93 349.1 195.2 384.8 177.71 413.78C156.99 448.1 116.34 456.94 79.05 440.88C79.05 440.88 139.13 471.23 192.6 438.56C192.6 438.56 229.48 415.97 231.75 362.32C234.02 308.68 197.65 274.13 130.82 233.21Z"
                                    fill="white"
                                />
                                <path
                                    d="M175.86 172.93C150.78 157.58 122.49 143.44 103.09 120.39C83.05 96.57 78.68 61.04 98.72 35.59C111.7 19.11 132.41 10.85 153.13 11.54C194.5 12.9 240.88 41.55 243.15 116.56H255.9V5.16999C255.9 5.16999 246.08 5.16999 246 5.16999C246 5.16999 244.25 12.75 244.1 13.4C243.19 17.3 242.17 22.22 239.19 25.1C235.81 28.37 230.65 28.59 226.33 27.45C209.05 22.9 189.05 -11.65 115.85 5.16999C57.72 18.53 36.75 84.34 55.74 136.25C78.39 198.13 150.03 213.05 195.41 252.95C266.79 312.46 245.06 375.95 245.05 375.97C252.15 370.11 257.89 362.56 262.96 354.93C269.87 344.53 276.02 332.84 278.25 320.49C290.96 250.04 228.17 204.95 175.86 172.93Z"
                                    fill="white"
                                />
                                <path
                                    d="M59.2517 587.986C57.8233 584.7 55.633 582.129 52.6809 580.272C49.7763 578.368 46.348 577.415 42.396 577.415C38.682 577.415 35.349 578.272 32.3968 579.987C29.4447 581.701 27.1116 584.153 25.3974 587.343C23.6833 590.486 22.8262 594.152 22.8262 598.342C22.8262 602.532 23.6833 606.222 25.3974 609.413C27.1116 612.603 29.4447 615.055 32.3968 616.769C35.349 618.483 38.682 619.34 42.396 619.34C45.8719 619.34 48.9907 618.602 51.7524 617.126C54.5617 615.603 56.7996 613.46 58.4661 610.698C60.1802 607.889 61.1563 604.627 61.3944 600.913H40.3962V596.842H66.6797V600.485C66.4416 604.865 65.2512 608.841 63.1086 612.412C60.9659 615.936 58.0852 618.721 54.4664 620.769C50.8953 622.816 46.8718 623.84 42.396 623.84C37.7773 623.84 33.5872 622.769 29.8256 620.626C26.064 618.436 23.0881 615.412 20.8978 611.555C18.7551 607.651 17.6838 603.247 17.6838 598.342C17.6838 593.438 18.7551 589.057 20.8978 585.2C23.0881 581.296 26.064 578.272 29.8256 576.13C33.5872 573.939 37.7773 572.844 42.396 572.844C47.7289 572.844 52.3952 574.178 56.3948 576.844C60.3945 579.51 63.299 583.224 65.1084 587.986H59.2517ZM102.407 623.34L90.0511 602.485H80.8375V623.34H75.838V573.416H90.8367C96.4077 573.416 100.622 574.749 103.479 577.415C106.383 580.082 107.835 583.582 107.835 587.914C107.835 591.533 106.788 594.628 104.693 597.199C102.645 599.723 99.5979 601.366 95.5506 602.128L108.407 623.34H102.407ZM80.8375 598.414H90.9081C94.8126 598.414 97.7409 597.461 99.6931 595.557C101.693 593.652 102.693 591.105 102.693 587.914C102.693 584.629 101.741 582.105 99.836 580.344C97.9313 578.534 94.9316 577.63 90.8367 577.63H80.8375V598.414ZM140.184 623.84C135.565 623.84 131.375 622.769 127.613 620.626C123.852 618.436 120.876 615.412 118.685 611.555C116.543 607.651 115.471 603.247 115.471 598.342C115.471 593.438 116.543 589.057 118.685 585.2C120.876 581.296 123.852 578.272 127.613 576.13C131.375 573.939 135.565 572.844 140.184 572.844C144.85 572.844 149.064 573.939 152.825 576.13C156.587 578.272 159.539 581.296 161.682 585.2C163.824 589.057 164.896 593.438 164.896 598.342C164.896 603.247 163.824 607.651 161.682 611.555C159.539 615.412 156.587 618.436 152.825 620.626C149.064 622.769 144.85 623.84 140.184 623.84ZM140.184 619.483C143.898 619.483 147.231 618.626 150.183 616.912C153.135 615.198 155.468 612.746 157.182 609.556C158.896 606.318 159.753 602.58 159.753 598.342C159.753 594.104 158.896 590.39 157.182 587.2C155.468 584.01 153.135 581.558 150.183 579.844C147.231 578.13 143.898 577.273 140.184 577.273C136.47 577.273 133.137 578.13 130.184 579.844C127.232 581.558 124.899 584.01 123.185 587.2C121.471 590.39 120.614 594.104 120.614 598.342C120.614 602.58 121.471 606.318 123.185 609.556C124.899 612.746 127.232 615.198 130.184 616.912C133.137 618.626 136.47 619.483 140.184 619.483ZM178.829 573.416V604.842C178.829 609.794 179.996 613.46 182.329 615.841C184.71 618.221 187.948 619.412 192.043 619.412C196.137 619.412 199.328 618.245 201.613 615.912C203.946 613.531 205.113 609.841 205.113 604.842V573.416H210.112V604.77C210.112 611.103 208.422 615.865 205.041 619.055C201.708 622.245 197.352 623.84 191.971 623.84C186.591 623.84 182.21 622.245 178.829 619.055C175.496 615.865 173.83 611.103 173.83 604.77V573.416H178.829ZM253.19 587.772C253.19 592.057 251.785 595.509 248.976 598.128C246.167 600.747 241.953 602.056 236.334 602.056H226.263V623.34H221.264V573.416H236.334C241.905 573.416 246.095 574.725 248.904 577.344C251.761 579.963 253.19 583.439 253.19 587.772ZM236.334 597.771C244.143 597.771 248.047 594.438 248.047 587.772C248.047 584.439 247.119 581.915 245.262 580.201C243.405 578.487 240.429 577.63 236.334 577.63H226.263V597.771H236.334Z"
                                    fill="white"
                                />
                                <path
                                    d="M305.68 619.02C309.789 619.02 313.12 615.689 313.12 611.58C313.12 607.471 309.789 604.14 305.68 604.14C301.571 604.14 298.24 607.471 298.24 611.58C298.24 615.689 301.571 619.02 305.68 619.02Z"
                                    fill="white"
                                />
                            </svg>
                        </div>
                        <div>
                            <h1 className="font-roboto text-white ">
                                Shella Saukia Group
                            </h1>
                        </div>
                        <div className="w-72">
                            <p className="font-roboto text-sm font-light text-white">
                                Jl. K.H. Wahid Hasyim No.161, RT.1/RW.5, Kb.
                                Kacang, Kecamatan Tanah Abang, Kota Jakarta
                                Pusat, Daerah Khusus Ibukota Jakarta 10240
                            </p>
                        </div>
                    </div>
                    <div>
                        <h1 className="text-white font-roboto text-sm mt-4 md:mt-16">
                            ©2023 Shella Saukia Group
                        </h1>
                    </div>
                </div>
                <div>
                    <h1 className="font-roboto text-white text-xl">Support</h1>
                    <ul className="">
                        <li className="font-roboto text-sm font-light text-white cursor-pointer hover:text-[#444341] mt-4">
                            Home
                        </li>
                        <li className="font-roboto text-sm font-light text-white cursor-pointer hover:text-[#444341] mt-4">
                            About Us
                        </li>
                        <li className="font-roboto text-sm font-light text-white cursor-pointer hover:text-[#444341] mt-4">
                            Faq
                        </li>
                        <li className="font-roboto text-sm font-light text-white cursor-pointer hover:text-[#444341] mt-4">
                            Career
                        </li>
                        <li className="font-roboto text-sm font-light text-white cursor-pointer hover:text-[#444341] mt-4">
                            Partner
                        </li>
                        <li className="font-roboto text-sm font-light text-white cursor-pointer hover:text-[#444341] mt-4">
                            Terms & Conditions
                        </li>
                        <li className="font-roboto text-sm font-light text-white cursor-pointer hover:text-[#444341] mt-4">
                            Privacy Policy
                        </li>
                    </ul>
                </div>
                <div className="flex flex-col justify-between">
                    <div>
                        <div>
                            <h1 className="font-roboto text-white text-xl">
                                Connect With Us
                            </h1>
                        </div>
                        <div className="flex items-center justify-between">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="40"
                                height="40"
                                viewBox="0 0 40 40"
                                fill="none"
                            >
                                <path
                                    d="M20 13.1667C18.6485 13.1667 17.3274 13.5674 16.2036 14.3183C15.0799 15.0691 14.204 16.1364 13.6868 17.385C13.1696 18.6336 13.0343 20.0076 13.298 21.3331C13.5616 22.6586 14.2125 23.8762 15.1681 24.8319C16.1238 25.7875 17.3414 26.4384 18.6669 26.702C19.9924 26.9657 21.3664 26.8304 22.615 26.3132C23.8636 25.796 24.9309 24.9201 25.6817 23.7964C26.4326 22.6726 26.8333 21.3515 26.8333 20C26.8355 19.102 26.6603 18.2124 26.3177 17.3824C25.975 16.5523 25.4718 15.7982 24.8368 15.1632C24.2018 14.5282 23.4477 14.025 22.6176 13.6823C21.7876 13.3397 20.898 13.1645 20 13.1667ZM20 24.45C19.1199 24.45 18.2595 24.189 17.5277 23.7C16.7959 23.2111 16.2256 22.5161 15.8887 21.7029C15.5519 20.8898 15.4638 19.995 15.6355 19.1318C15.8072 18.2686 16.231 17.4757 16.8534 16.8534C17.4757 16.231 18.2686 15.8072 19.1319 15.6355C19.9951 15.4638 20.8898 15.5519 21.703 15.8887C22.5161 16.2255 23.2111 16.7959 23.7001 17.5277C24.189 18.2595 24.45 19.1199 24.45 20C24.45 21.1802 23.9812 22.3121 23.1466 23.1466C22.3121 23.9811 21.1802 24.45 20 24.45ZM28.7167 12.8833C28.7167 13.213 28.6189 13.5352 28.4358 13.8093C28.2527 14.0833 27.9924 14.297 27.6878 14.4231C27.3833 14.5493 27.0482 14.5823 26.7249 14.518C26.4016 14.4537 26.1046 14.2949 25.8715 14.0618C25.6384 13.8287 25.4797 13.5318 25.4154 13.2085C25.3511 12.8852 25.3841 12.5501 25.5102 12.2455C25.6364 11.941 25.85 11.6807 26.1241 11.4975C26.3981 11.3144 26.7204 11.2167 27.05 11.2167C27.492 11.2167 27.916 11.3922 28.2285 11.7048C28.5411 12.0174 28.7167 12.4413 28.7167 12.8833ZM33.2333 14.55C33.2818 12.478 32.509 10.471 31.0833 8.96665C29.5871 7.5228 27.579 6.73158 25.5 6.76665C23.3333 6.66665 16.6667 6.66665 14.5 6.76665C12.4287 6.7218 10.423 7.49415 8.91668 8.91665C7.49107 10.421 6.7182 12.428 6.76668 14.5C6.66668 16.6667 6.66668 23.3333 6.76668 25.5C6.7182 27.5719 7.49107 29.579 8.91668 31.0833C10.423 32.5058 12.4287 33.2782 14.5 33.2333C16.7 33.3667 23.3 33.3667 25.5 33.2333C27.572 33.2818 29.579 32.5089 31.0833 31.0833C32.509 29.579 33.2818 27.5719 33.2333 25.5C33.3333 23.3333 33.3333 16.6667 33.2333 14.5V14.55ZM30.4 27.8833C30.1737 28.4565 29.8321 28.9772 29.3963 29.413C28.9606 29.8487 28.4399 30.1904 27.8667 30.4167C25.2793 30.9397 22.6343 31.1191 20 30.95C17.3713 31.1194 14.7317 30.94 12.15 30.4167C11.5768 30.1904 11.0561 29.8487 10.6204 29.413C10.1846 28.9772 9.84297 28.4565 9.61668 27.8833C8.91668 26.1167 9.06668 21.95 9.06668 20.0167C9.06668 18.0833 8.91668 13.9 9.61668 12.15C9.83659 11.5711 10.1758 11.0449 10.6122 10.6056C11.0486 10.1663 11.5726 9.82368 12.15 9.59998C14.7317 9.07661 17.3713 8.89728 20 9.06665C22.6343 8.89756 25.2793 9.07689 27.8667 9.59998C28.4399 9.82627 28.9606 10.1679 29.3963 10.6037C29.8321 11.0394 30.1737 11.5601 30.4 12.1333C31.1 13.9 30.9333 18.0667 30.9333 20C30.9333 21.9333 31.1 26.1166 30.4 27.8666V27.8833Z"
                                    fill="white"
                                />
                            </svg>
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="30"
                                height="30"
                                viewBox="0 0 30 30"
                                fill="none"
                            >
                                <path
                                    d="M26.25 9.89625V14.9388C23.9826 14.7138 21.8206 13.8702 20 12.5V18.125C19.9997 19.6514 19.5694 21.1469 18.7584 22.44C17.9474 23.7332 16.7885 24.7717 15.4146 25.4367C14.0406 26.1016 12.5071 26.3661 10.9898 26.1998C9.47249 26.0335 8.03269 25.4432 6.83538 24.4964C5.63807 23.5496 4.73167 22.2847 4.22003 20.8466C3.70839 19.4085 3.6122 17.8554 3.94249 16.3651C4.27277 14.8749 5.01616 13.5078 6.0875 12.4205C7.15884 11.3332 8.51479 10.5697 10 10.2175V15.625C9.57386 15.9446 9.23649 16.3678 9.01992 16.8545C8.80336 17.3412 8.71479 17.8751 8.76263 18.4056C8.81047 18.9362 8.99312 19.4457 9.29325 19.8857C9.59338 20.3258 10.001 20.6819 10.4775 20.9201C10.9539 21.1583 11.4833 21.2708 12.0155 21.2468C12.5476 21.2229 13.0648 21.0633 13.5179 20.7833C13.971 20.5032 14.3451 20.112 14.6044 19.6468C14.8638 19.1815 15 18.6577 15 18.125V3.75H20.1038C20.364 5.2855 21.0955 6.70203 22.1967 7.80328C23.298 8.90453 24.7145 9.63598 26.25 9.89625Z"
                                    stroke="#FFF8EB"
                                    strokeWidth="2"
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                />
                            </svg>
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="30"
                                height="30"
                                viewBox="0 0 30 30"
                                fill="none"
                            >
                                <path
                                    d="M17.5 15L13.125 17.5V12.5L17.5 15Z"
                                    fill="#FFF8EB"
                                    stroke="#FFF8EB"
                                    strokeWidth="2"
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                />
                                <path
                                    d="M2.5 15.8837V14.115C2.5 10.4962 2.5 8.68625 3.63125 7.5225C4.76375 6.3575 6.54625 6.3075 10.11 6.20625C11.7975 6.15875 13.5225 6.125 15 6.125C16.4763 6.125 18.2013 6.15875 19.89 6.20625C23.4538 6.3075 25.2363 6.3575 26.3675 7.5225C27.5 8.68625 27.5 10.4975 27.5 14.115V15.8837C27.5 19.5037 27.5 21.3125 26.3688 22.4775C25.2363 23.6412 23.455 23.6925 19.89 23.7925C18.2025 23.8412 16.4775 23.875 15 23.875C13.3697 23.8701 11.7395 23.8426 10.11 23.7925C6.54625 23.6925 4.76375 23.6425 3.63125 22.4775C2.5 21.3125 2.5 19.5025 2.5 15.885V15.8837Z"
                                    stroke="#FFF8EB"
                                    strokeWidth="2"
                                />
                            </svg>
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="30"
                                height="30"
                                viewBox="0 0 30 30"
                                fill="none"
                            >
                                <path
                                    d="M17.26 13.0863L26.1 2.5H24.005L16.3275 11.6913L10.1975 2.5H3.125L12.3975 16.4L3.125 27.5H5.22L13.3275 17.7938L19.8025 27.5H26.875L17.26 13.0863ZM14.39 16.5213L13.45 15.1375L5.975 4.125H9.19375L15.2262 13.0125L16.165 14.3963L24.0062 25.9488H20.7888L14.3887 16.5225L14.39 16.5213Z"
                                    fill="#FFF8EB"
                                />
                            </svg>
                        </div>
                    </div>
                    <div className="flex flex-col gap-1">
                        <h1 className="font-roboto text-white text-xl">
                            Customer Service
                        </h1>
                        <div className="flex items-center gap-2">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="30"
                                height="30"
                                viewBox="0 0 30 30"
                                fill="none"
                            >
                                <path
                                    d="M23.8125 6.13757C22.6665 4.97998 21.3015 4.06216 19.797 3.43764C18.2926 2.81312 16.6789 2.4944 15.05 2.50007C8.225 2.50007 2.6625 8.06257 2.6625 14.8876C2.6625 17.0751 3.2375 19.2001 4.3125 21.0751L2.5625 27.5001L9.125 25.7751C10.9375 26.7626 12.975 27.2876 15.05 27.2876C21.875 27.2876 27.4375 21.7251 27.4375 14.9001C27.4375 11.5876 26.15 8.47507 23.8125 6.13757ZM15.05 25.1876C13.2 25.1876 11.3875 24.6876 9.8 23.7501L9.425 23.5251L5.525 24.5501L6.5625 20.7501L6.3125 20.3626C5.28468 18.7213 4.73893 16.8241 4.7375 14.8876C4.7375 9.21257 9.3625 4.58757 15.0375 4.58757C17.7875 4.58757 20.375 5.66257 22.3125 7.61257C23.2719 8.56753 24.0321 9.7034 24.5492 10.9544C25.0664 12.2053 25.33 13.5465 25.325 14.9001C25.35 20.5751 20.725 25.1876 15.05 25.1876ZM20.7 17.4876C20.3875 17.3376 18.8625 16.5876 18.5875 16.4751C18.3 16.3751 18.1 16.3251 17.8875 16.6251C17.675 16.9376 17.0875 17.6376 16.9125 17.8376C16.7375 18.0501 16.55 18.0751 16.2375 17.9126C15.925 17.7626 14.925 17.4251 13.75 16.3751C12.825 15.5501 12.2125 14.5376 12.025 14.2251C11.85 13.9126 12 13.7501 12.1625 13.5876C12.3 13.4501 12.475 13.2251 12.625 13.0501C12.775 12.8751 12.8375 12.7376 12.9375 12.5376C13.0375 12.3251 12.9875 12.1501 12.9125 12.0001C12.8375 11.8501 12.2125 10.3251 11.9625 9.70007C11.7125 9.10007 11.45 9.17507 11.2625 9.16257H10.6625C10.45 9.16257 10.125 9.23757 9.8375 9.55007C9.5625 9.86257 8.7625 10.6126 8.7625 12.1376C8.7625 13.6626 9.875 15.1376 10.025 15.3376C10.175 15.5501 12.2125 18.6751 15.3125 20.0126C16.05 20.3376 16.625 20.5251 17.075 20.6626C17.8125 20.9001 18.4875 20.8626 19.025 20.7876C19.625 20.7001 20.8625 20.0376 21.1125 19.3126C21.375 18.5876 21.375 17.9751 21.2875 17.8376C21.2 17.7001 21.0125 17.6376 20.7 17.4876Z"
                                    fill="#FFF8EB"
                                />
                            </svg>
                            <h1 className="text-white font-roboto font-light cursor-pointer hover:text-[#444341]">
                                +62-821-123-456
                            </h1>
                        </div>
                        <div className="flex items-center gap-2">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="30"
                                height="30"
                                viewBox="0 0 30 30"
                                fill="none"
                            >
                                <path
                                    d="M27.5 7.5C27.5 6.125 26.375 5 25 5H5C3.625 5 2.5 6.125 2.5 7.5V22.5C2.5 23.875 3.625 25 5 25H25C26.375 25 27.5 23.875 27.5 22.5V7.5ZM25 7.5L15 13.75L5 7.5H25ZM25 22.5H5V10L15 16.25L25 10V22.5Z"
                                    fill="#FFF8EB"
                                />
                            </svg>
                            <h1 className="text-white font-roboto font-light cursor-pointer hover:text-[#444341]">
                                cs@ssgroup.id
                            </h1>
                        </div>
                        <div>
                            <h1 className="text-white font-roboto font-light">
                                Working Hours :
                            </h1>
                            <h1 className="text-white font-roboto font-light">
                                10:00 - 16:00 WIB
                            </h1>
                        </div>
                    </div>
                </div>
            </div>
        </>
    );
};

export default Footer;
