...
export const putArticle = (data, id) => async (dispatch) => {
    try {
        const headers = {
            Authorization: `Bearer ${Cookies.get("token")}`,
        };

        dispatch({ type: "PUT_ARTICLE_PENDING" });
        const result = await axios.put(url + "article/" + id, data, {
            headers,
        });
        console.log(result);
        dispatch({ payload: result.data.data, type: "PUT_ARTICLE_SUCCESS" });
    } catch (err) {
        console.error("error", err);
        const errorMessage =
            err.response && err.response.data
